...
from tests.seeds.community_seeds import create_seed_community_data
...
class TestCommunity(TestBase):
    ...
    def test_add_community(self, _, client: FlaskClient):
        ...
        response = client.get(url)
        self.assertLocationHeader(response, "/auth/auth?next=%2Fcommunities%2Fadd_community")
    ...
    def test_edit_community(self, _, client: FlaskClient):
        ...
        response = client.get(url)
        self.assertLocationHeader(response, "/auth/auth?next=%2Fcommunities%2FeditCommunity%2F1")
    ...
    def test_create_edit_delete_community(self, _, client: FlaskClient):
        ...
        self.assertEqual(created_community.category_id, int(community_data["category_id"]))
